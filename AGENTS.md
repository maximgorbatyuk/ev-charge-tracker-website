...
**Markdown for agents (`Accept: text/markdown`)**: This repo is static HTML on GitHub Pages; the origin cannot run content negotiation. When `evchargetracker.app` is served through **Cloudflare** (proxied DNS), enable Cloudflare **Markdown for Agents** so requests with `Accept: text/markdown` receive `Content-Type: text/markdown; charset=utf-8`, optional `x-markdown-tokens`, and `Vary: accept` on converted responses. No application code is required; see [Markdown for Agents](https://developers.cloudflare.com/fundamentals/reference/markdown-for-agents/). **Plan**: Pro, Business, or Enterprise (or SSL for SaaS per Cloudflare docs). **Dashboard**: zone → **AI** (AI Crawl Control) → enable **Markdown for Agents**. **API**: `PATCH /client/v4/zones/{zone_id}/settings/content_converter` with body `{"value":"on"}` and a token with Zone Settings — Edit. Optional path-scoped enablement: **Rules** → **Configuration rules** → add setting **Markdown for Agents** = On. Validate with `POST https://isitagentready.com/api/scan` and `checks.contentAccessibility.markdownNegotiation`.
...
- `robots.txt` — sitemap reference and [Content Signals](https://contentsignals.org/) (`Content-Signal: ai-train, search, ai-input` under `User-agent: *`)
- `favicon.ico`
